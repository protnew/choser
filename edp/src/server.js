...
import { councilEngineRoutes } from './api/councilEngine.js'
import { detectLang, getAvailableLocales, getLocale, t, createTranslator, SUPPORTED, DEFAULT } from './i18n/index.js'
      ...
      if (search && search.trim().length > 0) {
        try {
          ...
          results = db.prepare(`
            ...
            WHERE tables_fts MATCH ? AND t.state != 'deleted' AND t.visibility = 'open'
            ORDER BY rank LIMIT ?
          ...
          results = db.prepare(`
            SELECT * FROM tables
            WHERE (title LIKE ? OR description LIKE ? OR tags LIKE ?) AND state != 'deleted' AND visibility = 'open'
            ORDER BY updated_at DESC LIMIT ?
          `).all(likeSearch, likeSearch, likeSearch, limit)
        }
      } else {
        results = db.prepare(`
          SELECT * FROM tables WHERE state != 'deleted' AND visibility = 'open'
          ORDER BY updated_at DESC LIMIT ?
        `).all(limit)
      }
