...
export function buildTableFromVotes(votes, maxObjects = 0, maxParams = 0, tableWeights = {}) {
    const allScores = {};
    ...
    if (objNames.length > 0 && paramNames.length > 0) {
        const defaultWeight = Math.round(100 / paramNames.length);
        const columns = paramNames.map(p => ({
            name: p,
            weight: tableWeights[p] || defaultWeight
        }));
        ...
        const objects = objNames.map(name => {
            ...
            for (const p of paramNames) {
                ...
                if (d && d.grades.length > 0) {
                    ...
                    const roundedGrade = Math.max(0, Math.min(10, Math.round(avgGrade)));  // clamp to [0,10]
        ...
        // Sort by weighted average grade descending
        const totalW = columns.reduce((s, c) => s + c.weight, 0) || 1;
        objects.sort((a, b) => {
            const wAvgA = columns.reduce((s, c) => s + (a.scores[c.name]?.grade || 0) * c.weight, 0) / totalW;
            const wAvgB = columns.reduce((s, c) => s + (b.scores[c.name]?.grade || 0) * c.weight, 0) / totalW;
            return wAvgB - wAvgA;
        });
