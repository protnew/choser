...
console.log('Топ-10 параметров по весу:');
params.slice(0,10).forEach((p,i) => console.log(`  ${i+1}. ${p.title} (вес=${p.weight})`));
console.log();
...
console.log('--- BASELINE (полная модель) ---');
baseline.forEach((r, i) => console.log(`  ${i+1}. ${r.name} — ${r.score.toFixed(2)}`));
console.log();
...
const scenarios = [
  ['Без весов', noWeights],
  [`Топ-10 из ${params.length} параметров`, top10],
  [`Топ-5 из ${params.length} параметров`, top5],
  ['Бинарная шкала', binaryScores],
  ['3-балльная шкала', threeScores],
  ['Комбо (всё вместе)', combo],
];
console.log('Сценарий                              | Топ-3 | Лидер сменился? | Новый лидер');
console.log('---------------------------------------|-------|-----------------|------------');
for (const [label, scores] of scenarios) {
  ...
  console.log(`${label.padEnd(38)} | ${match}/3   | ${leaderChanged ? 'ДА' : 'нет'}            | ${leaderChanged ? mod3[0] : base3[0]}`);
}
