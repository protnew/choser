...
export default function Grid({ isEmbed }) {
        ...
        const loadData = async () => {
            ...
            try {
                if (tableId) {
                    ...
                    const baseUrl = '/v1/api/tables';
                    const params = new URLSearchParams({ limit: '200' });
                    if (searchQuery) params.set('q', searchQuery);
                    let resp = await API.get(`${baseUrl}?${params}`);
                    let data = resp.data || resp || [];
                    cursorRef.current = resp.cursor || null;
                    if (!resp.cursor) allLoadedRef.current = true;
    ...
    // --- Infinite Scroll: load more tables ---
    const loadMore = useCallback(async () => {
        if (loadingMore || allLoadedRef.current || !cursorRef.current || !isHome) return;
        setLoadingMore(true);
        try {
            const params = new URLSearchParams({ limit: '200', cursor: cursorRef.current });
            if (searchQuery) params.set('q', searchQuery);
            const resp = await API.get(`/v1/api/tables?${params}`);
            const newTables = (resp.data || []).map(t => ({ ...t, utility: (t.param_count || 0) * (t.object_count || 0) * (parseInt(t.views) || 1) }));
            cursorRef.current = resp.cursor || null;
            if (!resp.cursor) allLoadedRef.current = true;
            setRowData(prev => {
                const existingIds = new Set(prev.map(t => t.id));
                const filtered = newTables.filter(t => !existingIds.has(t.id));
                if (filter === 'my' && user) return [...prev, ...filtered.filter(t => t.author_id === user.id)];
                return [...prev, ...filtered];
            });
        } catch (e) { console.error('loadMore error:', e); }
        finally { setLoadingMore(false); }
    }, [loadingMore, isHome, searchQuery, filter, user]);
    ...
    const columnDefs = useMemo(() => {
        if (isHome) {
            const maxGlobalUtility = rowData.reduce((mx, r) => Math.max(mx, r.utility || 0), 0) || 5000;
            return [
        ...
        const edit = true;
        const maxPrice = rowData.reduce((mx, r) => Math.max(mx, parseFloat(r.price) || 0), 0) || 5000;
        const maxU = rowData.reduce((mx, r) => Math.max(mx, parseFloat(r._u) || 0), 0) || 1000;
        const maxUP = rowData.reduce((mx, r) => Math.max(mx, parseFloat(r._up) || 0), 0) || 15;
        const baseCols = [
    ...
    return (
        ...
        <div className={theme === 'dark' ? "ag-theme-quartz-dark" : "ag-theme-quartz"} style={{
            ...
            {isHome && allLoadedRef.current && rowData.length > 0 && !loading && (
                <div style={{ textAlign: 'center', padding: '8px', color: 'var(--text-muted)', fontSize: 12, opacity: 0.6 }}>
                    Всего загружено: {rowData.length} таблиц
                </div>
            )}
        </div>
