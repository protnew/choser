...
export default function Admin() {
    ...
    const loadUsers = async () => { setLoading(true); setUsers(await API.get('/api/admin/users')||[]); setLoading(false); };
    const loadArchive = async () => { setLoading(true); setArchivedTables(await API.get('/api/admin/archive')||[]); setLoading(false); };
    const loadSettings = async () => { setLoading(true); setSettings(await API.get('/api/admin/settings')||{system_prompt:''}); setLoading(false); };
    const loadSnapshots = async () => { setLoading(true); setSnapshots(await API.get('/api/admin/snapshots')||[]); setLoading(false); };
    const loadStats = async () => { setLoading(true); try{setStats(await API.get('/api/admin/stats-full'));}catch(e){console.error(e);} setLoading(false); };
    const loadCatalogObjects = async (q='') => { setLoading(true); setCatalogObjects(await API.get(`/api/catalog/objects${q?`?q=${encodeURIComponent(q)}`:''}`) || []); setLoading(false); };
    const loadCatalogParams = async (q='') => { setLoading(true); setCatalogParams(await API.get(`/api/catalog/params${q?`?q=${encodeURIComponent(q)}`:''}`) || []); setLoading(false); };
