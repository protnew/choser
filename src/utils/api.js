...
export const API = {
    ...
    async request(url, options = {}) {
        ...
        if (res.status === 401) {
            this.setToken(null);
            // HashRouter: navigate via hash, not pathname
            // Don't redirect to /login (doesn't exist) — just clear token and reload home
            if (!window.location.hash.includes('login')) {
                window.location.hash = '#/';
                window.location.reload();
            }
            throw new Error('Сессия истекла. Перезагрузка...');
        }
