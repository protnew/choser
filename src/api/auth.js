...
// POST /auth/dev-login — auto-login as admin (localhost/DEV only)
authRoutes.post('/auth/dev-login', async (c) => {
  const db = c.get('db')
  // Guard: only allow from local/private origins (blocks external access)
  const host = c.req.header('host') || ''
  const origin = c.req.header('origin') || ''
  const xForwardedFor = c.req.header('x-forwarded-for') || ''
  // Block if request comes from external (non-private) network
  if (xForwardedFor && !xForwardedFor.startsWith('127.') && !xForwardedFor.startsWith('10.') && !xForwardedFor.startsWith('172.') && !xForwardedFor.startsWith('192.168.')) {
    return c.json({ error: 'Dev login disabled for external requests' }, 403)
  }
  // Find existing admin (do NOT create new admin automatically)
